        ...
        for (let i = 0; i < 12; ++i) {
            ...
            for (let j = 0; j < 54; ++j) {
                if (this.cube.clockMoves[i][j] === 200) {
                    ...
                    const j2 = j1 + this.cube.clockMoves[i][j1];
                    const j3 = j2 + this.cube.clockMoves[i][j2];
                    const j4 = j3 + this.cube.clockMoves[i][j3];
                    if (j4 !== j) {
        ...
        for (let y = 1; y >= -1; --y) {
            for (let x = -1; x <= 1; ++x) {
                if (y === 1 && x === 5) {
                    ...
                    const tile1 = new Tile(x, y, tileIx, this.scene);
