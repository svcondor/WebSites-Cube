...
export class Solver {
    constructor(mainApp, cube) {
        ...
        this.whiteCross = (result) => {
            ...
            const frontColor = this.getTile(CubeFace.F, 4).color;
            ...
            const face = Math.floor(ixWhite / 9);
        ...
        this.whiteCorners = (result) => {
            const front = this.getTile(CubeFace.F, 4).color;
        ...
        this.orientateYellowCross = (result) => {
            ...
            for (let i = 0; i < 4; i++) {
                const j = (i + 3) % 4;
                const face = this.getTile(i, 1).color;
                const prevFace = this.getTile(j, 1).color;
            ...
            const front = this.getTile(CubeFace.F, 4).color;
            const top = this.getTile(CubeFace.F, 1).color;
    ...
    step(target = 0) {
        this.copyTilesFromCube();
        const result = { step: 1, subStep: 1, moves: "" };
        this.totalMoves = "";
        for (;;) {
            if (this.runStep(this.whiteCross, result))
                break;
            if (result.step < 3) {
                if (this.runStep(this.whiteCorners, result))
                    break;
            }
            if (this.runStep(this.middleSection, result))
                break;
            if (this.runStep(this.yellowCross, result))
                break;
            if (this.runStep(this.orientateYellowCross, result))
                break;
            if (this.runStep(this.yellowCorners, result))
                break;
            if (this.runStep(this.orientateYellowCorners, result))
                break;
            break;
        }
        if (result.moves !== "") {
    ...
    solverSendMoves(moves, execute = false, speed = this.cube.mainSpeed) {
        if (moves.length % 2 === 1) {
    ...
    findColors(color, color2, color3 = null) {
        if (color3 !== null) {
            for (let i = 0; i < Solver.cubeTable.length; i++) {
                const tile1 = this.getTile(i);
    ...
    copyTilesFromCube() {
        for (let ix = 0; ix < Cube.cubeTable.length; ++ix) {
            ...
            const tile1 = new Tile(-100, -100, tile.tileIx, null);
